...
def interface():
    ...
    while (response != '1') and (response != '2'):
        ...
        response = input(u"Press 1 to send a thank you note\n"
                         u"Press 2 to create a report ")
    if response == '1':
        name = input(u"Enter a donor\'s full name or enter the "
                     u"word 'list' to see a list of donors ")
        name = name.lower()
        while name == "list":
            for key in DONORS:
                print(key)
...
def create_report():
    ...
    average = report_average(report_totals(DONORS),
                             report_num_donations(DONORS))
    report_list = zip(names, totals, num_donations, average)
